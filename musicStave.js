class MusicStave extends HTMLElement {
    ...
    initSVG() {
        this.svg.setAttributeNS(null, "viewBox", `0 -2 100 ${this.NUMBER_OF_STAVE_POSITIONS + 4}`);
    ...
    drawClef(clefType) {
        ...
        const clef = this.svg.querySelector("#clef");
    ...
    markNextInLine() {
        if (this.markerGroups.length > 0) {
            ...
            next.setAttributeNS(null, "part", `${next.getAttributeNS(null, "part")} nextNote`);
        }
    }
    addLedgerLines(group) {
        const box = group.querySelector("circle").getBBox();
